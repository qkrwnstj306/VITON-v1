...
class LiTModel(pl.LightningModule):
    ...
    def __init__(self, diffusion, mlp, dinov2, decoder, encoder,
        ...
        # self.hook = CrossAttentionHook()
        # self.hook.take_module_for_loss(self.diffusion)
        # self.attention_loss_strength = 0.001
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            encoder_inputs = torch.cat((input_image, cloth_agnostic_mask, densepose), dim=0)
            
            encoder_noise = torch.randn((encoder_inputs.size(0), 4, LATENTS_HEIGHT, LATENTS_WIDTH))
            
            input_image_latents, cloth_agnostic_mask_latents, densepose_latents = \
        ...
        del input_image, cloth_agnostic_mask, densepose, cloth,  
        ...
        if self.args.use_attention_loss:
            naive_loss, cwg_loss, tv_loss, dcml_loss = self.sampler.train(self.diffusion, input_image_latents, cloth_agnostic_mask_latents,   
                             densepose_latents, image_embeddings, warped_cloth_mask, self.args.do_cfg, 
                             self.args.use_attention_loss)

            self.log('cwg_loss', cwg_loss, on_step=True, prog_bar=True, logger=True)
            self.log('tv_loss', tv_loss, on_step=True, prog_bar=True, logger=True)
            self.log('dcml_loss', dcml_loss, on_step=True, prog_bar=True, logger=True)
            loss = naive_loss + cwg_loss + tv_loss + dcml_loss
        
        else:
            loss = self.sampler.train(self.diffusion, input_image_latents, cloth_agnostic_mask_latents, 
                                      densepose_latents, image_embeddings, warped_cloth_mask=None, do_cfg=self.args.do_cfg, 
    ...
    def log_images(self, batch, batch_idx):
        ...
        cloth_agnostic_mask_latents, densepose_latents = \
            torch.chunk(self.encoder(encoder_inputs, encoder_noise.to("cuda")), 2, dim=0)
