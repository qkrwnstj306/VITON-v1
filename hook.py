...
class CrossAttentionHook(nn.Module):
    ...
    """From now on, below code is related with attention loss"""
    
    # def take_module_for_loss(self, model):
    #     for name, module in model.unet.named_modules():
    #         if isinstance(module, CrossAttention) and not 'bottleneck' in name:
    #             module.register_forward_hook(lambda m, inp, out, n=name: self.cross_attention_hook_for_loss(m, inp, out, n))
                
    # def cross_attention_hook_for_loss(self, module, input, output, name):
    #     # Get heat maps
    #     x, y = input[0], input[1]

    #     # [num_heads, context_seq_len, height, width]
    #     attention_loss = module.get_attention_loss(x, y)

    #     if type(self.cross_attention_forward_hooks_for_loss[name]) == int:
    #         self.cross_attention_forward_hooks_for_loss[name] = attention_loss
    #     else:
    #         self.cross_attention_forward_hooks_for_loss[name] += attention_loss
            
    # def make_attention_loss(self, cloth_mask):
        
    #     # [batch_size, num_heads, seq_len, height, width] * 15
    #     attention_maps = []

    #     for key in self.cross_attention_forward_hooks_for_loss.keys():
    #         attention_maps.append(self.cross_attention_forward_hooks_for_loss[key])

    #     # [batch_size, num_heads, seq_len, 37, 28] * 15 -> [batch_size, num_heads, 37, 28] * 15
    #     attention_maps = [attention_map.mean(dim=2) for attention_map in attention_maps]
    #     # [batch_size, num_heads * 15, 37, 28]
    #     attention_maps = torch.cat(attention_maps, dim=1)
    #     # [batch_size, 37, 28]: cloth attention maps 
    #     attention_maps = attention_maps.mean(1)

    #     cloth_mask = F.interpolate(cloth_mask, size=(37, 28), mode='nearest')

    #     cloth_region_attention_score = attention_maps * cloth_mask
        
    #     inverse_cloth_mask = 1 - cloth_mask 
    #     background_region_attention_score = attention_maps * inverse_cloth_mask
        
    #     # [batch_size, 1, 512, 384] 
    #     attention_loss = -1. * torch.log(cloth_region_attention_score.sum() + 1e-8 / (background_region_attention_score.sum() + 1e-8))
