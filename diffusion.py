...
class UNET_AttentionBlock(nn.Module):
    def __init__(self, n_head: int, n_embd: int, d_context=768, apply_attn_loss=False):
        ...
        self.attention_2 = CrossAttention(n_head, channels, d_context, apply_attn_loss=apply_attn_loss, in_proj_bias=False)
    ...
    def forward(self, x, context, warped_cloth_mask=None, use_attention_loss=False):
        ...
        if use_attention_loss:
            x, cwg_loss, tv_loss, dcml_loss = self.attention_2(x, context, use_attention_loss=use_attention_loss, warped_cloth_mask=warped_cloth_mask)
        else:
    ...
    def forward(self, x, context, time, warped_cloth_mask=None, use_attention_loss=False):
        ...
        for layer in self:
            if isinstance(layer, UNET_AttentionBlock):
                if use_attention_loss:
                    x, temp_cwg_loss, temp_tv_loss, temp_dcml_loss = layer(x, context, warped_cloth_mask, use_attention_loss=use_attention_loss)
...
class UNET(nn.Module):
    def __init__(self):
        ...
        self.encoders = nn.ModuleList([
            ...
            SwitchSequential(nn.Conv2d(12, 320, kernel_size=3, padding=1)),
    ...
    def forward(self, x, context, time, warped_cloth_mask=None, use_attention_loss=False):
        ...
        for layers in self.encoders:
            if use_attention_loss:
                x, temp_cwg_loss, temp_tv_loss, temp_dcml_loss = layers(x, context, time, warped_cloth_mask, use_attention_loss=use_attention_loss)
                cwg_loss += temp_cwg_loss
                tv_loss += temp_tv_loss
                dcml_loss += temp_dcml_loss
            else:    
        ...
        if use_attention_loss:
            x, temp_cwg_loss, temp_tv_loss, temp_dcml_loss = self.bottleneck(x, context, time, use_attention_loss=use_attention_loss)
            cwg_loss += temp_cwg_loss
            tv_loss += temp_tv_loss
            dcml_loss += temp_dcml_loss
        else:
...
class Diffusion(nn.Module):
    def __init__(self):
        ...
        self.uncond_vector = nn.Parameter(torch.randn((1,1037,768)), requires_grad=True)

    def forward(self, latent, cloth_agnostic_mask,
                densepose, image_embeddings, time, warped_cloth_mask=None, is_train=True, do_cfg=True, use_attention_loss=False):
        ...
        latent = torch.cat((latent, cloth_agnostic_mask, densepose), dim=1)
        ...
        if is_train and do_cfg:
            ...
            """
            cwg_loss, tv_loss, dcml_loss = torch.tensor(0, dtype=latent.dtype, device=latent.device), torch.tensor(0, dtype=latent.dtype, device=latent.device), torch.tensor(0, dtype=latent.dtype, device=latent.device)
            ...
            if probabilities < self.p_uncond:
                ...
                if use_attention_loss:
                    output, cwg_loss, tv_loss, dcml_loss = self.unet(latent, image_embeddings, time, warped_cloth_mask, use_attention_loss=use_attention_loss)
