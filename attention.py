...
class AttentionLoss(nn.Module):
    def __init__(self):
        ...
        self.cwg_strength = 0.0003
        self.dcml_strength = 0.01
        self.tv_strength = 0.0001 
    ...
    def get_cwgloss(self, reshaped_sim, weighted_centered_grid_hw, warped_cloth_mask, mh, mw, sigma=1.0):
        ...
        cwg_loss = -torch.sum(probabilities * reshaped_sim) / warped_cloth_mask.sum()
    ...
    def get_dcmlloss(self, center_coords, mask, cH, cW):
        ...
        # 행과 열 인덱스 생성 
        row_indices = torch.arange(center_coords.size(1)).view(-1, 1).repeat(1, center_coords.size(2)).view(-1)
        col_indices = torch.arange(center_coords.size(2)).view(-1, 1).repeat(center_coords.size(1), 1).view(-1)

        # [1, 192, 192], [1, 192, 192]
        row_exclude_mask = (row_indices.unsqueeze(1) == row_indices.unsqueeze(0)).unsqueeze(0)
        col_exclude_mask = (col_indices.unsqueeze(1) == col_indices.unsqueeze(0)).unsqueeze(0)
        
        # 모든 원소 값 쌍 간의 차이 계산, [BS, 192, 192]
        x_value_diffs = torch.abs(x_flat_values.unsqueeze(2) - x_flat_values.unsqueeze(1))  # shape: [BS, 192, 192]
        y_value_diffs = torch.abs(y_flat_values.unsqueeze(2) - y_flat_values.unsqueeze(1))  # shape: [BS, 192, 192]
        value_diffs = x_value_diffs * (~col_exclude_mask).float().cuda() + y_value_diffs * (~row_exclude_mask).float().cuda()
        """
...
class CrossAttention(nn.Module):
    ...
    def get_attention_score(self, x, y, generated_image=True):
        ...
        if generated_image:
            ...
            width = 28
            height = 37
            # [num_heads, seq_len_q - 1, height, width]
            weight = weight[...,1:]
    ...
    def forward(self, x, y, use_attention_loss = False, warped_cloth_mask = None, attention_type = "xformers"):
        ...
        if attention_type=="xformers":
            ...
            if self.apply_attn_loss and use_attention_loss and q.size(1) == 192 and warped_cloth_mask is not None:
                ...
                assert mH == 16 and mW == 12, "You Applied Attention Loss in Incorrect Cross-Attention Layer (Set to Index 6 or 7)"
                mh = 37
                mw = 28
                ...
                cwg_loss = attn_loss_class.get_cwgloss(reshaped_sim, weighted_centered_grid_hw, warped_cloth_mask, mh=mh, mw=mw, sigma=0.5)
